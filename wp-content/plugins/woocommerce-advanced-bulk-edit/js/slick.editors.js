...
//		  	   addsel = "";
		  	  if(prop.indexOf('attribute_pa_') === 0 && prop.indexOf('_ids') === -1 && prop.indexOf('_visiblefp') === -1)
			  {
				 ...
				 {
				 	for (var attr in W3Ex.attributes) 
					{
					  if (W3Ex.attributes.hasOwnProperty(attr)) 
					  {
					  	    if(attr === undefined) continue;
							var attrobj = W3Ex.attributes[attr];
							if(attrobj.attr === attr_slug)
							{
								if(selitem[prop + '_ids'] !== undefined)
								{
									...
									{
										if(attrobj.id == ids_arr[g])
										{
											addsel+= '<option value="'+attrobj.value+'">'+attrobj.name+'</option>';
											break;
										}
									}
								}
