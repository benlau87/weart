...
//	"use strict";
	...
	$('body').on('click','#bulkdialog .imgButton.sm',function()
	{
	  ...
	  var selecthtml = '<select id="selectmapfield">';
		...
		{
			var col = _idmap[i];
			if(col.type !== 'int' && col.type !== 'float2' && col.type !== 'float3'  && col.type !== 'text'  && col.type !== 'multitext')
				continue;
...
//			if($thirdtd.attr('data-mapped') === 'yes')
	...
	$('body').on('click','#bulkdialog .cancelmapfieldto',function()
	{
		$('#maptodialog').remove();
	});
...
//		$("#addok").button("disable");
	...
	var _idmap = [
		...
		{id:'_sku',field:'_sku',name:'SKU',scope:SCOPE.PRODSVAR,type:"text"},
