...
<div class="woocommerce_variation wc-metabox closed">
	<h3>
		<button type="button" class="remove_variation btn btn-sm btn-danger" rel="<?php echo esc_attr( $variation_id ); ?>"><?php _e( 'Remove', 'waa' ); ?></button>
		<div class="handlediv" title="<?php _e( 'Click to toggle', 'waa' ); ?>"></div>
		...
		<?php
			foreach ( $parent_data['attributes'] as $attribute ) {
				...
				echo '<select name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']"><option value="">' . __( 'Any', 'waa' ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
	...
	<table cellpadding="0" cellspacing="0" class="woocommerce_variable_attributes wc-metabox-content">
		<tbody>
			<tr>
				<td class="sku" colspan="2">
					<?php if ( wc_product_sku_enabled() ) : ?>
						...
						<label><?php _e( 'SKU', 'waa' ); ?>: <a class="tips" title="<?php _e( 'Enter a SKU for this variation or leave blank to use the parent product SKU.', 'waa' ); ?>" href="#">[?]</a></label>
					<?php else : ?>
					...
					<table cellspacing="0" cellpadding="0" class="data_table">
						<?php if ( get_option( 'woocommerce_manage_stock' ) == 'yes' ) : ?>
							<tr class="show_if_variation_manage_stock">
								<td>
									<label><?php _e( 'Stock Qty:', 'waa' ); ?> <a class="tips" title="<?php _e( 'Enter a quantity to enable stock management at variation level, or leave blank to use the parent product\'s options.', 'waa' ); ?>" href="#">[?]</a></label>
								...
								<td>
									...
									<select name="variable_backorders[<?php echo $loop; ?>]">
										<option value="no" <?php selected( $_backorders, 'no' ) ?>><?php _e( 'Do not allow', 'waa' ); ?></option>
										<option value="notify" <?php selected( $_backorders, 'notify' ) ?>><?php _e( 'Allow but notify customer', 'waa' ); ?></option>
										<option value="yes" <?php selected( $_backorders, 'yes' ) ?>><?php _e( 'Allow', 'waa' ); ?></option>
									</select>
								</td>
							</tr>
							<tr>
								<td colspan="2">
									<label><?php _e( 'Stock status', 'waa' ); ?> <a href="#" class="tips" title="<?php _e( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'waa' ) ?>">[?]</a></label>
									<select name="variable_stock_status[<?php echo $loop; ?>]">
										<option value="instock" <?php selected( $_stock_status, 'instock' ) ?>><?php _e( 'In stock', 'waa' ); ?></option>
										<option value="outofstock" <?php selected( $_stock_status, 'outofstock' ) ?>><?php _e( 'Out of stock', 'waa' ); ?></option>
									</select>
								</td>
							</tr>
						<?php endif; ?>
						...
						<tr class="variable_pricing">
							<td>
								<label><?php echo __( 'Regular Price:', 'waa' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?></label>
							...
							<td>
								<label><?php echo __( 'Sale Price:', 'waa' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?> <a href="#" class="sale_schedule"><?php _e( 'Schedule', 'waa' ); ?></a><a href="#" class="cancel_sale_schedule" style="display:none"><?php _e( 'Cancel schedule', 'woocommerce' ); ?></a></label>
							...
							<tr>
								<?php if ( wc_product_weight_enabled() ) : ?>
									<td class="hide_if_variation_virtual">
										<label><?php echo __( 'Weight', 'waa' ) . ' (' . esc_html( get_option( 'woocommerce_weight_unit' ) ) . '):'; ?> <a class="tips" title="<?php _e( 'Enter a weight for this variation or leave blank to use the parent product weight.', 'woocommerce' ); ?>" href="#">[?]</a></label>
						...
						<?php endif; ?>
						<tr>
							<td><label><?php _e( 'Shipping class:', 'waa' ); ?></label> <?php
								$args = array(
							...
							<td>
								...
								<select name="variable_tax_class[<?php echo $loop; ?>]">
									<option value="parent" <?php selected( is_null( $_tax_class ), true ); ?>><?php _e( 'Same as parent', 'waa' ); ?></option>
									<?php
									...
									<table class="widefat">
										<thead>
											<tr>
												...
												<td colspan="2"><?php _e( 'File URL', 'waa' ); ?> <span class="tips" title="<?php _e( 'This is the URL or absolute path to the file which customers will get access to.', 'woocommerce' ); ?>">[?]</span></td>
												<td>&nbsp;</td>
											</tr>
										</thead>
										<tfoot>
						...
						<tr class="show_if_variation_downloadable">
							<td>
								<div>
									...
									<input type="number" size="5" name="variable_download_limit[<?php echo $loop; ?>]" value="<?php if ( isset( $_download_limit ) ) echo esc_attr( $_download_limit ); ?>" placeholder="<?php _e( 'Unlimited', 'waa' ); ?>" step="1" min="0" />
								</div>
							</td>
							<td>
								<div>
									...
									<input type="number" size="5" name="variable_download_expiry[<?php echo $loop; ?>]" value="<?php if ( isset( $_download_expiry ) ) echo esc_attr( $_download_expiry ); ?>" placeholder="<?php _e( 'Unlimited', 'waa' ); ?>" step="1" min="0" />
								</div>
							</td>
						</tr>
			...
			<tr>
				...
				<td class="options">
					<label class="checkbox"><input type="checkbox" class="checkbox" name="variable_enabled[<?php echo $loop; ?>]" <?php checked( $variation_post_status, 'publish' ); ?> /> <?php _e( 'Enabled', 'waa' ); ?></label>

					<label class="checkbox"><input type="checkbox" class="checkbox variable_is_downloadable" name="variable_is_downloadable[<?php echo $loop; ?>]" <?php checked( isset( $_downloadable ) ? $_downloadable : '', 'yes' ); ?> /> <?php _e( 'Downloadable', 'waa' ); ?> <a class="tips" title="<?php _e( 'Enable this option if access is given to a downloadable file upon purchase of a product', 'waa' ); ?>" href="#">[?]</a></label>

					<label class="checkbox"><input type="checkbox" class="checkbox variable_is_virtual" name="variable_is_virtual[<?php echo $loop; ?>]" <?php checked( isset( $_virtual ) ? $_virtual : '', 'yes' ); ?> /> <?php _e( 'Virtual', 'waa' ); ?> <a class="tips" title="<?php _e( 'Enable this option if a product is not shipped or there is no shipping cost', 'waa' ); ?>" href="#">[?]</a></label>

					<label class="checkbox"><input type="checkbox" class="checkbox variable_manage_stock" name="variable_manage_stock[<?php echo $loop; ?>]" <?php checked( isset( $_manage_stock ) ? $_manage_stock : '', 'yes' ); ?> /> <?php _e( 'Manage Stock?', 'waa' ); ?> <a class="tips" title="<?php _e( 'Enable this option to enable stock management at variation level', 'waa' ); ?>" href="#">[?]</a></label>
