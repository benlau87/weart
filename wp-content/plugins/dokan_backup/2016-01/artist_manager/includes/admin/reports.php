<div class="wrap">
    ...
    $all_tabs = apply_filters( 'waa_admin_report_tabs', array(
        'report' => __( 'Reports', 'waa' ),
        'logs'   => __( 'All Logs', 'waa' ),
    ) );
    ?>

    <h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
        <?php foreach ( $all_tabs as $tab_id => $label ) { ?>
            <a href="admin.php?page=waa-reports&amp;tab=<?php echo esc_attr( $tab_id ); ?>" class="nav-tab<?php if ( $tab == $tab_id ) echo ' nav-tab-active'; ?>"><?php echo $label; ?></a>
        <?php } ?>
    </h2>

    <?php
    if ( $tab == 'report' ) {
        $report_sub_head = apply_filters( 'waa_admin_report_sub', array(
            'day'    => __( 'By Day', 'waa' ),
            'month'  => __( 'By Month', 'waa' ),
            'seller' => __( 'By Seller', 'waa' )
        ) );

        $head_count = count( $report_sub_head );
        $loop_count = 1;
        ?>
        <ul class="subsubsub waa-report-sub" style="float: none;">
            <?php foreach ( $report_sub_head as $sub_id => $sub_label ) { ?>
                <li>
                    <a href="admin.php?page=waa-reports&amp;tab=report&amp;type=<?php echo esc_attr( $sub_id ); ?>" <?php if ( $type == $sub_id ) echo 'class="current"'; ?>><?php echo $sub_label; ?></a>
        ...
        if ( isset( $_POST['waa_report_filter_date'] ) ) {
            $start_date = $_POST['start_date'];
            $end_date   = $_POST['end_date'];
        }

        if ( isset( $_POST['waa_report_filter_year'] ) ) {
        ...
        if ( $type == 'day' ) {
            ...
            <form method="post" class="form-inline report-filter" action="">
                ...
                <span class="form-group">
                    ...
                    <input type="submit" name="waa_report_filter_date" class="button button-primary" value="<?php _e( 'Show', 'waa' ); ?>" />
                </span>
            </form>
        ...
        if ( $type == 'seller' ) {
            ...
            $chosen_placeholder = __( 'Select a Store...', 'waa' );
            $seller_id          = isset( $_POST['chosen_store_name'] ) ? intval( $_POST['chosen_store_name'] ) : 0;
            ?>
            <form method="post" class="form-inline report-filter" action="" >

                <span class="form-group">
                    <label for='chosen_store_name'<b><?php _e( 'Store Name : ', 'waa' ); ?></b></label>
                    <select id='chosen_store_name' name='chosen_store_name' data-placeholder= '<?php echo __( 'Select a Store...', 'waa' ); ?>' style="width:350px;"  class="chosen-select">
                        <option></option>
                        <?php
        ...
        <?php } elseif ( $type == 'month' ) { ?>
            <form method="post" class="form-inline report-filter" action="">
                <span class="form-group">
                    <label for="from"><?php _e( 'Year:', 'waa' ); ?></label>
                ...
                <input type="submit" name="waa_report_filter_year" class="button button-primary" value="<?php _e( 'Show', 'waa' ); ?>" />
            </form>
            <?php
        } else {
            do_action( 'waa_report_sub_' . $type, $tab, $start_date, $end_date, $selected_year, $current_year );
        }
        ?>
        ...
        <div class="admin-report-container">
            ...
            if ( $type == 'month' ) {
                $report_data = waa_admin_report( 'month', $selected_year );
            } elseif ( $type == 'seller' ) {
                $report_data = waa_admin_report_by_seller( $chosen_seller_id );
            } else {
                $report_data = waa_admin_report();
            }
            ...
            <div class="waa-reports-sidebar">
                <ul class="chart-legend">
                    <li>
                        <strong><?php echo wc_price( $earning_total ); ?></strong>
                        <?php _e( 'Total Earning', 'waa' ); ?>
                    </li>
                    <li>
                        <strong><?php echo wc_price( $earning_total ); ?></strong>
                        <?php _e( 'Total Earning in this period', 'waa' ); ?>
                    </li>
                    <li>
                        <strong><?php echo wc_price( $order_total ); ?></strong>
                        <?php _e( 'Order total in this period', 'waa' ); ?>
                    </li>
                    <li>
                        <strong><?php echo $total_orders; ?></strong>
                        <?php _e( 'orders placed in this period', 'waa' ); ?>
                    </li>
                </ul>
            </div>
    ...
    <?php } else if ( $tab == 'logs' ) { ?>
        <?php
        $headers = array(
            'order_id'     => __( 'Order', 'waa' ),
            'seller_id'    => __( 'Seller', 'waa' ),
            'order_total'  => __( 'Order Total', 'waa' ),
            'net_amount'   => __( 'Seller Earning', 'waa' ),
            'commision'    => __( 'Commision', 'waa' ),
            'order_status' => __( 'Status', 'waa' ),
        );
        $headers = apply_filters( 'waa_earning_report_header', $headers );
        $statuses = wc_get_order_statuses();
        ?>
        <?php do_action( 'waa_prev_report_form', $_GET ); ?>
        ...
        <table class="widefat withdraw-table" style="margin-top: 15px;">
            ...
            <tbody>
                ...
                $sql = "SELECT do.*, p.post_date FROM {$wpdb->prefix}waa_orders do
                ...
                foreach ( $all_logs as $log ) {
                    ...
                    $result = array(
                        ...
                        'seller_id'    => '<a href="' . add_query_arg( array( 'seller_id' => $log->seller_id ) ) . '">' . $seller->display_name . '</a> (<a href="' . admin_url( 'user-edit.php?user_id=' . $log->seller_id ) . '">' . __( 'edit', 'waa' ) . '</a>)',
                    ...
                    $result                = apply_filters( 'waa_report_table_value', $result, $log->order_id, $log->seller_id );
        ...
        <div class="tablenav bottom">
            ...
            if ( $all_logs ) {
                ...
                $count        = $wpdb->get_var( "SELECT COUNT(id) FROM {$wpdb->prefix}waa_orders WHERE $count_where" );
    ...
    <?php } else { ?>
        <?php do_action( 'waa_admin_report_tab_' . $tab ); ?>
    <?php } ?>
