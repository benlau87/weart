 ...
 * @return void
 */
function waa_variable_product_type_options() {
    ...
    $tax_class_options[''] = __( 'Standard', 'waa' );
    if ( $tax_classes ) {
    ...
    <div id="variable_product_options" class="wc-metaboxes-wrapper">
        ...
        <?php if ( ! $variation_attribute_found ) : ?>
            ...
            <div id="message" class="inline woocommerce-message">
                <div class="squeezer">
                    <h4><?php _e( 'Before adding variations, add and save some attributes on the <strong>Attributes</strong> tab.', 'waa' ); ?></h4>

                    <p class="submit"><a class="button-primary" href="http://docs.woothemes.com/document/product-variations/" target="_blank"><?php _e( 'Learn more', 'waa' ); ?></a></p>
                </div>
            </div>
        ...
        <?php else : ?>
            ...
            <div class="woocommerce_variations wc-metaboxes">
                ...
                if ( $variations )  {
                    ...
                    foreach ( $variations as $variation ) {
                        ...
                        include waa_INC_DIR . '/woo-views/variation-admin-html.php';
            ...
            <p class="toolbar">
                ...
                <button type="button" class="btn btn-sm btn-success button-primary add_variation" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Add Variation', 'waa' ); ?></button>

                <button type="button" class="btn btn-sm btn-default link_all_variations" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Link all variations', 'waa' ); ?></button>

                <strong><?php _e( 'Default selections:', 'waa' ); ?></strong>
                <?php
                    ...
                    foreach ( $attributes as $attribute ) {
                        ...
                        echo '<select name="default_attribute_' . sanitize_title( $attribute['name'] ) . '"><option value="">' . __( 'No default', 'waa' ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
    ...
    jQuery(function($){
        ...
        jQuery('#variable_product_options').on('click', 'button.link_all_variations', function(){
            ...
            var answer = confirm('<?php echo esc_js( __( 'Are you sure you want to link all variations? This will create a new variation for each and every possible combination of variation attributes (max 50 per run).', 'waa' ) ); ?>');
            ...
            if (answer) {
                ...
                var data = {
                    action: 'waa_link_all_variations',
                    post_id: <?php echo $post->ID; ?>,
                ...
                jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
                    ...
                    if (count==1) {
                        alert( count + ' <?php echo esc_js( __( "variation added", 'waa' ) ); ?>');
                    } else if (count==0 || count>1) {
                        alert( count + ' <?php echo esc_js( __( "variations added", 'waa' ) ); ?>');
                    } else {
                        alert('<?php echo esc_js( __( "No variations added", 'waa' ) ); ?>');
                    }
        ...
        jQuery('#variable_product_options').on('click', 'button.remove_variation', function(e){
            e.preventDefault();
            var answer = confirm('<?php echo esc_js( __( 'Are you sure you want to remove this variation?', 'waa' ) ); ?>');
            if (answer){
        ...
        wp.media.view.settings.post = <?php echo json_encode( array( 'param' => 'waa', 'post_id' => $post->ID ) ); // big juicy hack. ?>;
        ...
        jQuery('#variable_product_options').on('click', '.upload_image_button', function( event ) {
            ...
            if ( $button.is('.remove') ) {
                ...
                variable_image_frame = wp.media.frames.variable_image = wp.media({
                    ...
                    title: '<?php echo esc_js( __( 'Choose an image', 'waa' ) ); ?>',
                    button: {
                        text: '<?php echo esc_js( __( 'Set variation image', 'waa' ) ); ?>'
                    }
                });
 ...
 * @return void
 */
function waa_process_product_meta( $post_id ) {
  ...
  #  }
    ...
    if ( $new_sku == '' ) {
        ...
        if ( ! empty( $new_sku ) ) {
            if (
                ...
                $woocommerce_errors[] = __( 'Product SKU must be unique.', 'waa' );
            } else {
    ...
    if ( $product_type == 'variable' )
        waa_save_variations( $post_id );

    // Do action for product type
    do_action( 'woocommerce_process_product_meta_' . $product_type, $post_id );
    do_action( 'waa_process_product_meta', $post_id );

    // Clear cache/transients
    wc_delete_product_transients( $post_id );
}

function waa_new_process_product_meta( $post_id ) {
    global $wpdb, $woocommerce, $woocommerce_errors;
...
/*    if ( isset( $_POST['_required_tax'] ) && $_POST['_required_tax'] == 'yes' ) {
           ...
           update_post_meta( $post_id, '_overwrite_shipping', 'yes' );
            ...
            if (isset($_POST['dps_country_to_price'])) {
                ...
                foreach ($_POST['dps_country_to'] as $key => $value) {
                    ...
                    if (!empty($value)) {
                        $rates[$country] = waa_get_woocs_int_price($c_price);
                    }
                }
            }
    ...
    // Save variations
    if ( $product_type == 'variable' ) {
        waa_new_save_variations( $post_id );
    } else {
        // Get variations
        $args = array(
            'post_type'     => 'product_variation',
...
function waa_new_save_variations( $post_id ) {
    ...
    if ( isset( $_POST['variable_sku'] ) ) {
        ...
        if( isset( $_POST['waa_create_new_variations'] ) && $_POST['waa_create_new_variations'] == 'yes' ) {
            $max_loop = max( array_keys( $variable_menu_order ) );

            for ( $i = 0; $i <= $max_loop; $i ++ ) {
                $post_status = isset( $variable_enabled[ $i ] ) ? 'publish' : 'private';
                $variation_ids = waa_create_new_variation( $post_id, $post_status );
...
function waa_create_new_variation( $post_id, $post_status ) {
...
function waa_save_variations( $post_id ) {
    global $woocommerce, $wpdb;
 ...
 * @return void
 */
function waa_create_sub_order( $parent_order_id ) {
    ...
    foreach ($sellers as $seller_id => $seller_products ) {
        waa_create_seller_order( $parent_order, $seller_id, $seller_products );
    }
}

add_action( 'woocommerce_checkout_update_order_meta', 'waa_create_sub_order' );
 ...
 */
function waa_create_seller_order( $parent_order, $seller_id, $seller_products ) {
    $order_data = apply_filters( 'woocommerce_new_order_data', array(
        'post_type'     => 'shop_order',
        'post_title'    => sprintf( __( 'Order &ndash; %s', 'waa' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'waa' ) ) ),
    ...
    if ( $order_id && !is_wp_error( $order_id ) ) {
        ...
        // do shipping
        $shipping_cost = waa_create_sub_order_shipping( $parent_order, $order_id, $seller_products );

        // add coupons if any
        waa_create_sub_order_coupon( $parent_order, $order_id, $product_ids );
        $discount = waa_sub_order_get_total_coupon( $order_id );
        ...
        do_action( 'waa_checkout_update_order_meta', $order_id, $seller_id );
    } // if order
}
 ...
 * @return int
 */
function waa_sub_order_get_total_coupon( $order_id ) {
    global $wpdb;
 ...
 * @return type
 */
function waa_create_sub_order_coupon( $parent_order, $order_id, $product_ids ) {
 ...
 * @return type
 */
function waa_create_sub_order_shipping( $parent_order, $order_id, $seller_products ) {
    ...
    $shipping_method = apply_filters( 'waa_shipping_method', $shipping_method, $order_id, $parent_order );
 ...
 * @return \WP_Error
 */
function waa_seller_registration_errors( $error ) {
    $allowed_roles = apply_filters( 'waa_register_user_role', array( 'customer', 'seller' ) );

    // is the role name allowed or user is trying to manipulate?
    if ( isset( $_POST['role'] ) && !in_array( $_POST['role'], $allowed_roles ) ) {
        return new WP_Error( 'role-error', __( 'Cheating, eh?', 'waa' ) );
    }
    ...
    if ( $role == 'seller' ) {
        ...
        if ( empty( $first_name ) ) {
            return new WP_Error( 'fname-error', __( 'Please enter your first name.', 'waa' ) );
        }

        $last_name = trim( $_POST['lname'] );
        if ( empty( $last_name ) ) {
            return new WP_Error( 'lname-error', __( 'Please enter your last name.', 'waa' ) );
        }
        $phone = trim( $_POST['phone'] );
        if ( empty( $phone ) ) {
            return new WP_Error( 'phone-error', __( 'Please enter your phone number.', 'waa' ) );
        }
...
add_filter( 'woocommerce_process_registration_errors', 'waa_seller_registration_errors' );
add_filter( 'registration_errors', 'waa_seller_registration_errors' );
 ...
 * @return array
 */
function waa_new_customer_data( $data ) {
...
add_filter( 'woocommerce_new_customer_data', 'waa_new_customer_data');



/**
 * Adds default waa store settings when a new seller registers
 *
 * @param int $user_id
 * @param array $data
 * @return void
 */
function waa_on_create_seller( $user_id, $data ) {
    if ( $data['role'] != 'seller' ) {
        return;
    }
    ...
    update_user_meta( $user_id, 'waa_profile_settings', $waa_settings );

    waa_Email::init()->new_seller_registered_mail( $user_id );
}

add_action( 'woocommerce_created_customer', 'waa_on_create_seller', 10, 2);
 ...
 * @return \WP_Query
 */
function waa_get_featured_products( $per_page = 9) {
    $featured_query = new WP_Query( apply_filters( 'waa_get_featured_products', array(
 ...
 * @return \WP_Query
 */
function waa_get_latest_products( $per_page = 9) {
    $featured_query = new WP_Query( apply_filters( 'waa_get_featured_products', array(
        'posts_per_page'      => $per_page,
        'post_type'           => 'product',
        'ignore_sticky_posts' => 1,
 ...
 * @return \WP_Query
 */
function waa_get_best_selling_products( $per_page = 8 ) {
    ...
    $best_selling_query = new WP_Query( apply_filters( 'waa_best_selling_query', $args ) );
 ...
 * @return \WP_Query
 */
function waa_get_top_rated_products( $per_page = 8 ) {
    ...
    $top_rated_query = new WP_Query( apply_filters( 'waa_top_rated_query', $args ) );
...
function waa_get_on_sale_products( $per_page = 10, $paged = 1 ) {
    ...
    return new WP_Query( apply_filters( 'waa_on_sale_products_query', $args ) );
}
 ...
 * @return mixed
 */
function waa_get_seller_balance( $seller_id, $formatted = true ) {
    global $wpdb;

    $status        = waa_withdraw_get_active_order_status_in_comma();
    $cache_key     = 'waa_seller_balance_' . $seller_id;
    $earning       = wp_cache_get( $cache_key, 'waa' );
    $threshold_day = waa_get_option( 'withdraw_date_limit', 'waa_selling', 0 );
    $date          = date( 'Y-m-d', strtotime( date('Y-m-d') . ' -'.$threshold_day.' days' ) );

    if ( false === $earning ) {
        $sql = "SELECT SUM(net_amount) as earnings,
            (SELECT SUM(amount) FROM {$wpdb->prefix}waa_withdraw WHERE user_id = %d AND status = 1) as withdraw
            FROM {$wpdb->prefix}waa_orders as do LEFT JOIN {$wpdb->prefix}posts as p ON do.order_id = p.ID
        ...
        wp_cache_set( $cache_key, $earning, 'waa' );
    }
 ...
 * @return type
 */
function waa_get_seller_rating( $seller_id ) {
    global $wpdb;
 ...
 * @return void
 */
function waa_get_readable_seller_rating( $seller_id ) {
    $rating = waa_get_seller_rating( $seller_id );

    if ( ! $rating['count'] ) {
        echo __( 'No ratings found yet!', 'waa' );
        return;
    }

    $long_text = _n( __( '%s rating from %d review', 'waa' ), __( '%s rating from %d reviews', 'waa' ), $rating['count'], 'waa' );
    $text = sprintf( __( 'Rated %s out of %d', 'waa' ), $rating['rating'], number_format( 5 ) );
    ...
    ?>
        ...
        <span class="text"><a href="<?php echo waa_get_review_url( $seller_id ); ?>"><?php printf( $long_text, $rating['rating'], $rating['count'] ); ?></a></span>
 ...
 * @return void
 */
function waa_user_update_to_seller( $user, $data ) {
    ...
    if ( waa_get_option( 'new_seller_enable_selling', 'waa_selling', 'on' ) == 'off' ) {
    ...
    $publishing = waa_get_option( 'product_status', 'waa_selling' );
    $percentage = waa_get_option( 'seller_percentage', 'waa_selling' );
 ...
 * @return void
 */
function waa_become_seller_handler () {
    if ( isset( $_POST['waa_migration'] ) && wp_verify_nonce( $_POST['waa_nonce'], 'account_migration' ) ) {
...
add_action( 'template_redirect', 'waa_become_seller_handler' );
 ...
 * @return array
 */
function waa_exclude_child_customer_receipt( &$phpmailer ) {
    $subject      = $phpmailer->Subject;

    // order receipt
    $sub_receipt  = __( 'Your {site_title} order receipt from {order_date}', 'waa' );
    $sub_download = __( 'Your {site_title} order from {order_date} is complete', 'waa' );
    ...
    if ( isset( $matches[1] ) ) {
        ...
        if ( ! is_wp_error( $order ) && $order->post_parent != 0 ) {
            $phpmailer = new waaFakeMailer();
        }
    }
}

add_action( 'phpmailer_init', 'waa_exclude_child_customer_receipt' );
