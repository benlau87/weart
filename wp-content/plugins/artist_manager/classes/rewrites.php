...
class waa_Rewrites {
    ...
    public function __construct() {
        $this->custom_store_url = waa_get_option( 'custom_store_url', 'waa_selling', 'store' );
    ...
    function register_rule() {
        $this->query_vars = apply_filters( 'waa_query_var_filter', array(
            'products',
            'new-product',
            'orders',
            'coupons',
        ...
        do_action( 'waa_rewrite_rules_loaded' );
    }
    ...
    function store_template( $template ) {
        ...
        if ( !empty( $store_name ) ) {
            ...
            if ( ! waa_is_user_seller( $store_user->ID ) ) {
                return get_404_template();
            }

            return waa_locate_template( 'store.php' );
        }
    ...
    function store_toc_template( $template ) {
        ...
        if ( get_query_var( 'toc' ) ) {
            return waa_locate_template( 'store-toc.php' );
        }
    ...
    function product_edit_template( $template ) {
        ...
        if ( get_query_var( 'edit' ) && is_singular( 'product' ) ) {
            if ( waa_get_option( 'product_style', 'waa_selling', 'old' ) == 'old' ) {
                $edit_product_url = waa_get_template_part( 'product-edit' );
            } elseif ( waa_get_option( 'product_style', 'waa_selling', 'old' ) == 'new' ) {
                $edit_product_url = waa_get_template_part( 'new-product-single' );
            }
    ...
    function store_review_template( $template ) {
        ...
        if ( get_query_var( 'store_review' ) ) {
            return waa_locate_template( 'store-reviews.php' );
        }
    ...
    function account_migration_template( $file ) {
        ...
        if ( get_query_var( 'account-migration' ) && waa_is_user_customer( get_current_user_id() ) ) {
            return waa_locate_template( 'update-account.php' );
        }
