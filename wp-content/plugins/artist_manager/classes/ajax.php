...
class waa_Ajax {
    ...
    function init_ajax() {
        ...
        // reviews
        $reviews = waa_Template_reviews::init();
        add_action( 'wp_ajax_waa_comment_status', array( $reviews, 'ajax_comment_status' ) );
        add_action( 'wp_ajax_waa_update_comment', array( $reviews, 'ajax_update_comment' ) );

        //settings
        $settings = waa_Template_Settings::init();
        add_action( 'wp_ajax_waa_settings', array( $settings, 'ajax_settings' ) );

        add_action( 'wp_ajax_waa-mark-order-complete', array( $this, 'complete_order' ) );
        add_action( 'wp_ajax_waa-mark-order-processing', array( $this, 'process_order' ) );
        add_action( 'wp_ajax_waa_grant_access_to_download', array( $this, 'grant_access_to_download' ) );

        add_action( 'wp_ajax_waa_change_status', array( $this, 'change_order_status' ) );

        add_action( 'wp_ajax_waa_contact_seller', array( $this, 'contact_seller' ) );

        add_action( 'wp_ajax_waa_add_variation', array( $this, 'add_variation' ) );
        add_action( 'wp_ajax_waa_link_all_variations', array( $this, 'link_all_variations' ) );
        add_action( 'wp_ajax_waa_pre_define_attribute', array( $this, 'waa_pre_define_attribute' ) );
        add_action( 'wp_ajax_waa_save_attributes', array( $this, 'save_attributes' ) );

        add_action( 'wp_ajax_waa_toggle_seller', array( $this, 'toggle_seller_status' ) );
        ...
        add_action( 'wp_ajax_waa_announcement_remove_row', array( $this, 'remove_announcement') );
        add_action( 'wp_ajax_nopriv_waa_announcement_remove_row', array( $this, 'remove_announcement') );

        // shipping state ajax
        add_action( 'wp_ajax_nopriv_waa_shipping_country_select', array( $this, 'get_state_by_shipping_country') );
        add_action( 'wp_ajax_waa_shipping_country_select', array( $this, 'get_state_by_shipping_country') );

        // shipping calculation ajax
        add_action( 'wp_ajax_nopriv_waa_shipping_calculator', array( $this, 'get_calculated_shipping_cost') );
        add_action( 'wp_ajax_waa_shipping_calculator', array( $this, 'get_calculated_shipping_cost') );

        // Single product Design ajax
        add_action( 'wp_ajax_waa_save_attributes_options', array( $this, 'save_attributes_options') );
        add_action( 'wp_ajax_nopriv_waa_save_attributes_options', array( $this, 'save_attributes_options') );
        add_action( 'wp_ajax_waa_save_variations_options', array( $this, 'save_variations_options') );
        add_action( 'wp_ajax_nopriv_waa_save_variations_options', array( $this, 'save_variations_options') );
        add_action( 'wp_ajax_waa_add_new_variations_options', array( $this, 'add_new_variations_options') );
        add_action( 'wp_ajax_nopriv_waa_add_new_variations_options', array( $this, 'add_new_variations_options') );
        add_action( 'wp_ajax_waa_remove_single_variation_item', array( $this, 'remove_single_variation_item') );
        add_action( 'wp_ajax_nopriv_waa_remove_single_variation_item', array( $this, 'remove_single_variation_item') );
        add_action( 'wp_ajax_waa_get_pre_attribute', array( $this, 'add_predefined_attribute') );
        add_action( 'wp_ajax_nopriv_waa_get_pre_attribute', array( $this, 'add_predefined_attribute') );
    ...
    function seller_info_checkout( $item_data, $cart_item ) {
        $info   = waa_get_store_info( $cart_item['data']->post->post_author );
        $seller = sprintf( __( '<br><strong> Seller:</strong> %s', 'waa' ), $info['store_name'] );
        $data   = $item_data . $seller;

        return apply_filters( 'waa_seller_info_checkout', $data, $info, $item_data, $cart_item );
    }
    ...
    function shop_url_check() {
        ...
        if ( !wp_verify_nonce( $_POST['_nonce'], 'waa_reviews' ) ) {
            wp_send_json_error( array(
                'type' => 'nonce',
                'message' => __( 'Are you cheating?', 'waa' )
            ) );
        }
        ...
        if ( is_user_logged_in() && waa_is_user_customer( $user_ID ) ) {
    ...
    function complete_order() {
        ...
        if ( !current_user_can( 'dokandar' ) || waa_get_option( 'order_status_change', 'waa_selling', 'on' ) != 'on' ) {
            wp_die( __( 'You do not have sufficient permissions to access this page.', 'waa' ) );
        }

        if ( !check_admin_referer( 'waa-mark-order-complete' ) ) {
            wp_die( __( 'You have taken too long. Please go back and retry.', 'waa' ) );
        }
        ...
        if ( !waa_is_seller_has_order( get_current_user_id(), $order_id ) ) {
            wp_die( __( 'You do not have permission to change this order', 'waa' ) );
        }
    ...
    function process_order() {
        ...
        if ( !check_admin_referer( 'waa-mark-order-processing' ) ) {
            wp_die( __( 'You have taken too long. Please go back and retry.', 'waa' ) );
        }
    ...
    function change_order_status() {
        ...
        check_ajax_referer( 'waa_change_status' );
        ...
        $status_class = waa_get_order_status_class( $order_status );

        echo '<label class="waa-label waa-label-' . $status_class . '">' . $status_label . '</label>';
        exit;
    }
    ...
    function contact_seller() {
        ...
        check_ajax_referer( 'waa_contact_seller' );
        // print_r($posted);

        $seller = get_user_by( 'id', (int) $posted['seller_id'] );

        if ( !$seller ) {
            $message = sprintf( '<div class="alert alert-success">%s</div>', __( 'Something went wrong!', 'waa' ) );
            wp_send_json_error( $message );
        }

        $contact_name = trim( strip_tags( $posted['name'] ) );

        waa_Email::init()->contact_seller( $seller->user_email, $contact_name, $posted['email'], $posted['message'] );

        $success = sprintf( '<div class="alert alert-success">%s</div>', __( 'Email sent successfully!', 'waa' ) );
        wp_send_json_success( $success );
        exit;
    }


    function waa_pre_define_attribute() {
        ...
        <div class="inputs-box woocommerce_attribute" data-count="<?php echo $i; ?>">
            <div class="box-header">
                ...
                <span class="actions">
                    <button class="row-remove btn pull-right btn-danger btn-sm"><?php _e( 'Remove', 'waa' ); ?></button>
                </span>
            </div>
                    ...
                    <ul class="list-unstyled ">
                        <li>
                            <label class="checkbox-inline">
                                ...
                                ?> name="attribute_visibility[<?php echo $i; ?>]" value="1" /> <?php _e( 'Visible on the product page', 'waa' ); ?>
                            </label>
                        </li>
                        ...
                        ?> name="attribute_variation[<?php echo $i; ?>]" value="1" /> <?php _e( 'Used for variations', 'waa' ); ?></label>
                        </li>
                    </ul>
                </div>
                    ...
                    <ul class="option-couplet list-unstyled ">
                        ...
                        if ($options) {
                            foreach ($options as $count => $option) {
                                ?>
                                <li>
                                    <input type="text" class="option" placeholder="<?php _e( 'Option...', 'waa' ); ?>" name="attribute_values[<?php echo $i; ?>][<?php echo $count; ?>]" value="<?php echo esc_attr( $option->name ); ?>">
                            ...
                            ?>
                            <li>
                                <input type="text" class="option" name="attribute_values[<?php echo $i; ?>][0]" placeholder="<?php _e( 'Option...', 'waa' ); ?>">
                                <span class="item-action actions">
                                    <a href="#" class="row-add">+</a>
                                    <a href="#" class="row-remove">-</a>
                                </span>
    ...
    function add_predefined_attribute() {
        ...
        ob_start();
        ?>
        <tr class="waa-<?php echo $single; ?>attribute-options">
            <td width="20%">
                <input type="text" disabled="disabled" value="<?php echo $attr_name; ?>" class="waa-form-control waa-<?php echo $single; ?>attribute-option-name-label" data-attribute_name="<?php echo wc_sanitize_taxonomy_name( str_replace( 'pa_', '', $attribute_taxonomy_name ) ); ?>">
                <input type="hidden" name="attribute_names[]" value="<?php echo esc_attr( $attribute_taxonomy_name ); ?>" class="waa-<?php echo $single; ?>attribute-option-name">
                <input type="hidden" name="attribute_is_taxonomy[]" value="1">
            </td>
            <td colspan="3"><input type="text" name="attribute_values[]" value="<?php echo implode( ',', $att_val ); ?>" class="waa-form-control waa-<?php echo $single; ?>attribute-option-values"></td>
            <td><button class="waa-btn waa-btn-theme remove_<?php echo $remove_btn; ?>attribute"><i class="ui ui-trash-o"></i></button></td>
        </tr>
        <?php
    ...
    function toggle_seller_status() {
        ...
        if ( $user_id && in_array( $status, array( 'yes', 'no' ) ) ) {
            update_user_meta( $user_id, 'waa_enable_selling', $status );
            ...
            ?>
             <tr>
                <td>
                    ...
                    <select name="dps_state_to[<?php echo $country_id ?>][]" class="waa-form-control dps_state_selection" id="dps_state_selection">
                        <?php waa_state_dropdown( $states[$country_id], '', true ); ?>
                    </select>
                </td>
                <td>
                    ...
                    <div class="waa-input-group">
                        ...
                        <input type="text" placeholder="0.00" class="waa-form-control" name="dps_state_to_price[<?php echo $country_id; ?>][]">
                    </div>
                </td>
                <td width="15%">
                    <label for=""></label>
    ...
    function remove_announcement() {
        ...
        check_ajax_referer( 'waa_reviews' );
    ...
    function get_state_by_shipping_country() {
        ...
        if ( isset( $dps_state_rates[$country] ) && count( $dps_state_rates[$country] ) ) { ?>
            <label for="waa-shipping-state" class="waa-control-label"><?php _e( 'State', 'waa' ); ?></label>
            <select name="waa-shipping-state" class="waa-shipping-state waa-form-control" id="waa-shipping-state">
                <option value=""><?php _e( '--Select State--', 'waa' ); ?></option>
    ...
    function get_calculated_shipping_cost() {
        ...
        if ( $country != '' ) {
            if ( isset( $dps_state_rates[$country] ) && count( $dps_state_rates[$country] ) && empty( $_POST['state'] ) ) {
                _e( 'Please select a State from the dropdown', 'waa' );
            } else if ( !isset( $dps_state_rates[$country] ) && empty( $_POST['state'] ) ) {
                echo __( 'Shipping Cost : ', 'waa' ) . '<h4>' . wc_price( $dps_country_rates[$country] + $base_shipping_type_price + $additional_quantity_cost ) . '</h4>';
            } else if ( isset( $_POST['state'] ) && !empty( $_POST['state'] ) ) {
                $state = $_POST['state'];
                echo __( 'Shipping Cost : ', 'waa' ) . '<h4>' . wc_price( $dps_state_rates[$country][$state] + $base_shipping_type_price + $additional_quantity_cost ) . '</h4>';
            }
        } else {
            _e( 'Please select a country from the dropdown', 'waa' );
        }
        $content = ob_get_clean();
