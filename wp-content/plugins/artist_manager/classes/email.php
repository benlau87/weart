...
class waa_Email {
    ...
    function admin_email() {
        return apply_filters( 'waa_email_admin_mail', get_option( 'admin_email' ) );
    }
    ...
    function contact_seller( $seller_email, $from_name, $from_email, $message ) {
        $template = waa_INC_DIR . '/emails/contact-seller.php';
        ob_start();
        include $template;
        $body = ob_get_clean();
        ...
        $subject = sprintf( __( '"%s" sent you a message from your "%s" store', 'waa' ), $from_name, $this->get_from_name() );
    ...
    function prepare_withdraw( $body, $user, $amount, $method, $note = '' ) {
        ...
        $replace = array(
            ...
            waa_withdraw_get_method_title( $method ),
            admin_url( 'user-edit.php?user_id=' . $user->ID ),
            admin_url( 'admin.php?page=waa-withdraw' ),
            $this->get_from_name(),
            home_url(),
            $note
        );
    ...
    function new_withdraw_request( $user, $amount, $method ) {
        $template = waa_INC_DIR . '/emails/withdraw-new.php';
        ob_start();
        include $template;
        $body = ob_get_clean();

        $subject = sprintf( __( '[%s] New Withdraw Request', 'waa' ), $this->get_from_name() );
    ...
    function withdraw_request_approve( $user_id, $amount, $method ) {
        $template = waa_INC_DIR . '/emails/withdraw-approve.php';
        ob_start();
        include $template;
        $body = ob_get_clean();

        $user = get_user_by( 'id', $user_id );
        $subject = sprintf( __( '[%s] Your Withdraw Request has been approved', 'waa' ), $this->get_from_name() );
        $body = $this->prepare_withdraw( $body, $user, $amount, $method );
    ...
    function withdraw_request_cancel( $user_id, $amount, $method, $note = '' ) {
        $template = waa_INC_DIR . '/emails/withdraw-cancel.php';
        ob_start();
        include $template;
        $body = ob_get_clean();

        $user = get_user_by( 'id', $user_id );
        $subject = sprintf( __( '[%s] Your Withdraw Request has been cancelled', 'waa' ), $this->get_from_name() );
    ...
    function new_seller_registered_mail( $seller_id ) {
        $template = waa_INC_DIR . '/emails/new-seller-registered.php';
        ...
        $body = str_replace( $find, $replace, $body);
        $subject = sprintf( __( '[%s] New Seller Registered', 'waa' ), $this->get_from_name() );
    ...
    function new_product_added( $product_id, $status = 'pending' ) {
        $template = waa_INC_DIR . '/emails/new-product-pending.php';

        if ( $status == 'publish' ) {
            $template = waa_INC_DIR . '/emails/new-product.php';
        }
        ob_start();
        include $template;
        $body = ob_get_clean();
        ...
        $body = str_replace( $find, $replace, $body);
        $subject = sprintf( __( '[%s] New Product Added', 'waa' ), $this->get_from_name() );
    ...
    function product_published( $post, $seller ) {
        ...
        $template = waa_INC_DIR . '/emails/product-published.php';
        ...
        $replace = array(
            ...
            waa_edit_product_url( $post->ID ),
            $this->get_from_name(),
            home_url(),
        );

        $body = str_replace( $find, $replace, $body);
        $subject = sprintf( __( '[%s] Your product has been approved!', 'waa' ), $this->get_from_name() );
