...
<td class="upload_image" style="width:2% !important;">
    ...
    <a href="#" class="upload_image_button <?php if ( $image_id > 0 ) echo 'waa-img-remove'; ?>" rel="<?php echo esc_attr( $variation_id ); ?>">
...
<td>
    <?php
        foreach ( $parent_data['attributes'] as $attribute ) {
            ...
            echo '<select name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']" class="waa-w3 waa-form-control"><option value="">' . __( 'Any', 'waa' ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
...
<td style="width:10% !important;">
    <?php if ( wc_product_sku_enabled() ) : ?>
        <input type="text" size="5" class="waa-form-control" name="variable_sku[<?php echo $loop; ?>]" value="<?php if ( isset( $_sku ) ) echo esc_attr( $_sku ); ?>" placeholder="<?php echo esc_attr( $parent_data['sku'] ); ?>" size="10"/>
    <?php else : ?>
...
<td style="width:25% !important;">
    ...
    <a class="waa-btn waa-btn-theme remove_variation" data-variation_id=<?php echo $variation_id; ?>><i class="fa fa-trash-o"></i></a>
</td>
