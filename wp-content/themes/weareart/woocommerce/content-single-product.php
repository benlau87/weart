...
#print_r($product);
$artist_id = $product->post->post_author;
$product_processing_time = get_post_meta($product->id, '_dps_processing_time', true);
$dps_processing = get_user_meta($artist_id, '_dps_pt', true);
$processing_time = ($product_processing_time) ? $product_processing_time : $dps_processing;
$artist_has_pickup = artist_has_pickup($artist_id);
$country_obj = new WC_Countries();
$countries = $country_obj->countries;
$from = get_user_meta($artist_id, '_dps_form_location', true);
?>
...
<div itemscope itemtype="<?php echo woocommerce_get_product_schema(); ?>"
    ...
    <div class="row">
        ...
        <div class="col-md-4 product-sidebar">
            ...
            $author = get_user_by('id', $artist_id);
            ...
            if ($processing_time) { ?>
                ...
                <br/>
            <?php }
            if ($artist_has_pickup) {
                $artist_location = $author_info['address']['city'];
                ?>
                <em><?php _e('Selbstabholung möglich', 'waa'); ?> <span
                        class="waa-tooltips-help tips" title=""
                        data-original-title="<?php printf(__('Das Kunstwerk kann in %s (%s) abgeholt werden. Die genaue Adresse erhältst du nach Abschluss des Kaufes.', 'waa'), $artist_location, $countries[$from]); ?>"><i
                            class="ui ui-question-circle"></i></span></em>
                <br/>
    ...
    <div class="artist-block">
        <div class="artist-info">
            ...
            <div class="artist-shortdesc">
				<span
                    style="font-weight:bold; font-style:italic">&raquo; <?= the_title() . ' &laquo;</span> ' . __('von', 'waa'); ?>
                    <a href="<?= waa_get_store_url($author->ID) ?>"
                       title="<?php printf(__('Profil von %s ansehen	', 'waa'), $author_info['store_name']); ?>">
                    ...
                    if (is_object($city_term))
                        printf(__('(<em>Künstler aus</em> <a href="%1$s" title="weitere Künstler aus %2$s anzeigen">%2$s</a>)', 'waa'), $city_link, $city_term->name); ?>
                    <?php
                    the_content();
